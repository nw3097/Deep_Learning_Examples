# relative to my_mnist_9.py: clean up main function, modularize things
...
# Import data
  ...
  return train_dataset, eval_dataset, iterator, next_element

def inference(image):
  W_conv1 = weight_variable([5, 5, 1, 32]) # weights of first conv layer
  b_conv1 = bias_variable([32]) # biases of first conv layer
  x_image = tf.reshape(image, [-1,28,28,1]) # reshape x into 2d images
  ...
  h_fc1 = tf.nn.relu(tf.matmul(h_pool2_flat, W_fc1) + b_fc1) # output of first fully connected layer # dropout probability
...
def main(_):  
  # load data
  train_dataset, eval_dataset, iterator, next_element = import_data()
  
  # define initialization operations by dataset type
  training_init_op = iterator.make_initializer(train_dataset)
  eval_init_op = iterator.make_initializer(eval_dataset)

  # define train op and accuracy
  y_conv = inference(next_element[0])
  cross_entropy = loss(labels=next_element[1], logits=y_conv) # loss operation
